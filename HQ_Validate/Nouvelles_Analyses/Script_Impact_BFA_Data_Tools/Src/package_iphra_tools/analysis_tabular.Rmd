...
```{r logo, echo=FALSE}
htmltools::img(src = knitr::image_uri("./../resources/Logo_Reach_RGB_1.png"), #./../resources/
               alt = "REACH logo",
...
```{r setup, include=FALSE}
   ...
   file.exists("iphra_analysis/src/init.R")
   source(here::here("iphra_analysis/src/init.R"))

  ##############################################################################################################

  ## FIX A FEW THINGS TO ALIGN DATA & TOOL WITH ANALYSIS SCRIPT
  source(here::here("iphra_analysis/src/fix_bugged_names.R"))
  ## ADDING WEIGHTS, OVERALL AND GROUPING VARIABLES 

  source(here::here("iphra_analysis/src/format_dataset.R"))
...
```{r setup2, include=FALSE}
 ...
 # CONVERT!
  # source("iphra_analysis/src/convert_cols_with_daf.R")

source(here::here("iphra_analysis/src/convert_cols_with_daf.R"))
  ## SRVYR DESIGNS
  ##############################################################################
...
```{r, include = FALSE} 
data_nut_import <- readxl::read_excel(here::here("iphra_analysis/data/BFA_ACF_IPHRA_PIELA_final_anonymized_data241230_pdi.xlsx"), sheet = "child_nutrition") %>% 
    filter(
        nut_child_present %in% c("yes", "disabled") #09-01-25 modif
      )

nut <- nut %>% select(-c("sex")) %>% 
  left_join(data_nut_import %>% select(loop_index, sex = child_sex), by = "loop_index") %>% 
  mutate(
    sex = case_when(
      sex == "homme" ~ "Male", T ~ "Female"
    )
  )
if(!is.null(nut)){
  ...
  num_sample <- results_prop_age_group %>% 
    mutate(num_sample = ifelse(Name %in% c("gam_muac_noflag"), 0, num_sample)) %>% #num sample modifier le 09-01-2025
    filter(!is.na(age_group)) %>% 
    group_by(age_group) %>% 
  ...
  if(all(c("nut_edema_confirm","nut_muac_cm") %in% names(nut))){
    nut_assessment <- nut %>% 
      ...
      mutate(oedema_less_2 = ifelse(nut_edema_confirm == "yes"& age_months < 24,1,0),
             muac_115_less_2 = ifelse(nut_muac_cm<11.5 & age_months < 24,1,0),#sam -- ok
             muac_115_124_less_2 = ifelse(nut_muac_cm>=11.5&nut_muac_cm<=12.4& age_months < 24,1,0),#mam -ok
             muac_125_less_2 = ifelse(nut_muac_cm>12.4& age_months < 24,1,0),#normal
             oedema_more_2 = ifelse(nut_edema_confirm == "yes"& age_months >= 24,1,0),#sam
