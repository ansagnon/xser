...
if(!is.null(ind_health)){
  ind_health <- ind_health %>% 
    dplyr::mutate(age_group = as.character(cut(as.numeric(health_ind_age_years), 
                                  ...
                                  labels = c("00-02", "03-05", "06-10", "11-18",
                                             "19-59","59+"))),
                  health_ind_sex = dplyr::case_when(health_ind_sex =="homme"~"homme",
                                                    health_ind_sex =="femme"~"femme"))
}
if(!is.null(child_nutrition)){
  child_nutrition_data <- child_nutrition %>%
    # filter(
    #   nut_child_present %in% c("yes", "disabled") #09-01-25 modif
    # ) %>% 
    dplyr::mutate(age_group_nut = as.character(cut(as.numeric(child_age_months), 
                                  breaks = c(5,23,59),
                                  labels = c("06-23", "24-59"))),
                  child_sex = dplyr::case_when(child_sex =="homme"~"homme",
...
if(all(hdds_check_columns %in% names(main))) {
  ...
  for(i in hdds_check_columns){
    ...
    res.wide <- res.long %>% tidyr::pivot_wider(names_from = !!rlang::sym(i), values_from = c(prop),
                                         values_fill = "0%") %>% 
      select(-prop_var) #%>% 
    if("no" %in% names(res.wide)){
      res.wide <- res.wide %>% 
        rename(No = "no")
    }else{
      if("yes" %in% names(res.wide)){
        res.wide <- res.wide %>% 
          rename(Yes = "yes")
      }
    }
    ...
    hdds_table <- bind_rows(hdds_table,res.wide)#rbind(hdds_table,res.wide)
  }
...
if(all(hhs_check_columns %in% names(main))) {
  ...
  for(i in hhs_column){
    ...
    hhs_table <- bind_rows(hhs_table,res.wide)#rbind(hhs_table,res.wide)
  }
  ...
  for(i in hhs_column_Freq){
    ...
    res.wide <- res.long %>% tidyr::pivot_wider(names_from = !!rlang::sym(i), values_from = c(prop),
      ...
      rename(#Often = "often",
             ...
             Sometimes = "sometimes") %>% 
      relocate(HHS, .before=1)
    hhs_tabl_Freq <- bind_rows(hhs_tabl_Freq,res.wide)#rbind(hhs_tabl_Freq,res.wide)
  }
...
if(all(fcs_check_columns %in% names(main))) {
  ...
  for(i in fcs_check_columns){
    ...
    fcs_table <- bind_rows(fcs_table,res.long)#rbind(fcs_table,res.long)
  }
...
if(all(rcsi_check_columns %in% names(main))) {
  ...
  for(i in rcsi_check_columns){
    ...
    rcsi_table <- bind_rows(rcsi_table,res.long)#rbind(rcsi_table,res.long)
  }
...
if(all(lcsi_check_columns %in% names(main))) {
  ...
  for(i in lcsi_check_columns){
    ...
    lcsi_table <- bind_rows(lcsi_table,res.wide)#rbind(lcsi_table,res.wide)
  }
...
if(!is.null(died_member)){
  ...
  results.summary <- bind_rows(crude,under5) %>% 
    # rbind(crude,under5) %>% 
    select(-x_deff) %>% 
    rename(`point.est` = "x",
           `95%lci` = "x_low",
  ...
  results.summary_bySex <- bind_rows(crude_m,
                                     crude_f,
                                     under5_m,
                                     under5_f) %>%
    # rbind(crude_m,
